...
public class Main extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == about) {
			JOptionPane.showMessageDialog(this, "斗地主小游戏");
		}
		...
		if (e.getSource() == publishCard[0]) {
			...
			/** 给点选的牌排序 */
			/*
			 * for(int i=0;i<c.size();i++){ System.out.println("点选的牌是："+c.get(i).name); }
			 */
	...
	private void restart() {
		...
		/*
		 * for(int i=0;i<3;i++){ currentList[i]=null; } for(int i=0;i<3;i++){
		 * playerList[i]=null; } lordList=null; for(int i=0;i<card.length;i++){
		 * card[i]=null; } nextPlayer=false;
		 */
